...
API.prototype.getAccessToken = async function (ifForce) {
	...
	if (!token || ifForce) {
		var url = this.prefix + 'oauth/token?grant_type=silent&client_id=' + this.client_id + '&client_secret=' + this.client_secret + '&kdt_id=' + this.kdt_id;

		var data = await this.request(url, { dataType: 'json' }).then(function (res) {
			return res.data;
		});
...
API.prototype.invoke = async function (apiName) {
	...
	var _opt$version = opt.version,
	    ...
	    _opt$dataType = opt.dataType,
	    dataType = _opt$dataType === undefined ? 'json' : _opt$dataType;
	...
	url += 'api/oauthentry/' + service + '/' + version + '/' + action + '?access_token=' + token.accessToken;

	console.log('url, data', url, opt.data);
	return this.request(url, extend({ dataType: dataType }, opt)).then(function (res) {
		var data = res.data;
		var error_response = data.error_response;
